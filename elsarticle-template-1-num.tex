...
\author{Roy Emanuel II, Ph.D.\corref{cor1}}
...
\cite{emanuel2018,emanuel2019} to accommodate discrete-time
...
Endogenous preference, $Q$, is the amount of the functional output
that a stakeholder desires at a given time \cite{Black2013}.  Many
models of resilience assume the stakeholder's endogenous preference
...
framework allows for time and disturbance dependent endogenous
...
$\Delta T$ factors are segments of time before system failure begins ($\Delta
...
The study represents the operations of a flight training squadron with
a discrete event simulation, so a discrete
representation of the resilience model is necessary to use the
simulation's outputs. Eq. \ref{e:contResilience} remains unchanged,
but the 
each profiles (Eqs. \ref{e:RPC} must be modified as follows to accommodate
...
Each stakeholder defines their own preference profile including time
horizon ($t_h$), endogenous preference ($Q_t$), and intertemporal
substitutability ($\chi$). The resilience model applies the preference
profile to the system performance ($\varphi_t$) over time to yield a dimensionless
value for resilience. Resilience has a minimum of zero and no
maximum. 
...
initiates a service life extension plan for the aircraft. As each
...
\begin{table}[h]
\begin{center}
    \caption{Simulation Parameter Starting Values }
...
SLEP line. The simulation assigns each aircraft a flight limit ranging from 3,500  to 7,000 flight hours to prevent
...
in the ``Surge'' scenario. The solid line is the number of aircraft on the
